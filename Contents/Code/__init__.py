...
class HamaCommonAgent:
  ...
  def Update(self, metadata, media, lang, force, movie):
    ...
    elif metadata_source.startswith("tmdb"):  tmdbid = metadata_id_number #                        #in ["tmdb", "tsdb"]:
    ...
    if imdbid.isdigit():                                                                                           # After tvdb.get_tvdb_metadata()
      if (Prefs['GetOmdbPoster'    ]):                                                                             omdb.omdb_poster         (metadata, imdbid)                                    ### OMDB - Posters - Using imdbid ###  return 200 but not downloaded correctly - IMDB has a single poster, downloading through OMDB xml, prefered by mapping file
      if not tmdbid:                                                                                               tmdbid = tmdb.get_tmdbid_per_imdbid(imdbid, tmdbid)                            # Updates tmdbid from imdbid, Used by fanart.tv
    if tmdbid.isdigit() and movie:                                                                                 tmdb.tmdb_tagline(metadata, movie, tmdbid)                                     ### Populate Movie Metadata Extras (e.g. Taglines) from TMDB for Movies ###
    ...
    if (tvdbid.isdigit() or tmdbid.isdigit()) and (Prefs['GetFanartTVPoster'] or Prefs['GetFanartTVBackground']):  FanartTv.fanarttv_posters(metadata, movie, tmdbid, tvdbid, defaulttvdbseason)  ### fanart.tv - Background, Poster and Banner - Using imdbid ###
