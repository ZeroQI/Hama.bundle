...
def Start():
  ...
  if not AniDB_title_tree:        Log.Fatal("Failed to load core file '%s'" % os.path.splitext(os.path.basename(ANIDB_TITLES))[0]); raise Exception("HAMA Fatal Error Hit") #; AniDB_title_tree = XML.ElementFromString("<animetitles></animetitles>")
  ...
  if not AniDB_TVDB_mapping_tree: Log.Fatal("Failed to load core file '%s'" % os.path.basename(ANIDB_TVDB_MAPPING));                raise Exception("HAMA Fatal Error Hit") #; AniDB_TVDB_mapping_tree = XML.ElementFromString("<anime-list></anime-list>")
...
class HamaCommonAgent:
  ...
  def Search(self, results, media, lang, manual, movie):
    ...
    if match:  ###metadata id provided
      ...
      Log.Info( "search - source: '%s', id: '%s', show from id: '%s' provided in foldername: '%s'" % (source, guid, show, orig_title) )
    ...
    for element in AniDB_title_tree_elements:
      if element.get('aid'):
        if score: #only when match found and it skipped to next serie in file, then add
          ...
          Log.Info("search() - AniDB - score: '%3d', id: '%6s', title: '%s' " % (score, aid, show))
        ...
        title = element.text
        if   title.lower()              == orig_title.lower() and 100                            > score:  parent_element, show , score = element.getparent(), title,         100; Log.Info("search() - AniDB - temp score: '%3d', id: '%6s', title: '%s' " % (100, aid, show))  #match = [element.getparent(), show,         100]
    ...
    if maxi<50:
      try:                    TVDBsearchXml = XML.ElementFromURL( TVDB_SERIE_SEARCH + orig_title.replace(" ", "%20"), cacheTime=CACHE_1HOUR * 24)
      except Exception as e:  Log.Error("search() - TVDB Loading search XML failed, Exception: '%s'" % e)
      else:
        for serie in TVDBsearchXml.xpath('Series'):
          ...
          Log.Info( "search() - TVDB  - score: '%3d', id: '%6s', title: '%s'" % (score, serie.xpath('seriesid')[0].text, serie.xpath('SeriesName')[0].text) )
    ...
    ### TMDB movie search ###
    Log.Info("search() - TMDB  - url: " + TMDB_MOVIE_SEARCH % orig_title)  #config_dict = self.get_json(TMDB_CONFIG_URL, cache_time=CACHE_1WEEK * 2)
    try:                    tmdb_json = JSON.ObjectFromURL(TMDB_MOVIE_SEARCH % orig_title.replace(" ", "%20"), sleep=2.0, headers={'Accept': 'application/json'}, cacheTime=CACHE_1WEEK * 2)
      ...
      if isinstance(tmdb_json, dict) and 'results' in tmdb_json:
        for i, movie in enumerate(tmdb_json['results']):
          ...
          score = 100 - 100*Util.LevenshteinDistance(a,b) / max(len(a),len(b)) if a!=b else 100
          id = movie['id']
          Log.Info( "search() - TMDB  - score: '%3d', id: '%6s', title: '%s'" % (score, movie['id'],  movie['title']) )
  ...
  def Update(self, metadata, media, lang, force, movie):
    ...
    Log.Info("Update - metadata source: '%s', id: '%s', Title: '%s',(%s, %s, %s)" % (metadata_id_source, metadata_id_number, metadata.title, "[...]", "[...]", force) )
    ...
    if metadata_id_source_core == "tvdb":  tvdbid = metadata_id_number
      ...
      Log.Info("Update() - TMDB  - url: " + TMDB_MOVIE_SEARCH_BY_TMDBID % tmdbid)
      ...
      except Exception as e:  Log.Error("Update() - get_json - Error fetching JSON page '%s', Exception: '%s'" %(TMDB_MOVIE_SEARCH_BY_TMDBID % tmdbid, e))
      else:
    ...
    if tvdbid.isdigit(): ### TVDB ID exists ####
      ...
      if THEME_URL % tvdbid in metadata.themes:  Log.Info("Update() - Theme song - already added")
      else:
      ...
      Log.Info("Update() - TVDB - tvdbid: '%s', url: '%s'" %(tvdbid, TVDB_HTTP_API_URL % tvdbid))
      ...
      if tvdbanime:
        ...
        if imdbid is None or imdbid =="" and getElementText(tvdbanime, 'Series/IMDB_ID'):  imdbid = getElementText(tvdbanime, 'Series/IMDB_ID');  Log.Warn("Update() - IMDB ID was empty, loaded through tvdb serie xml, IMDBID: '%s'" % imdbid)
      ...
      if Prefs['GetTvdbPosters'] or Prefs['GetTvdbFanart' ] or Prefs['GetTvdbBanners']:
        ...
        if tvdbposternumber * tvdbseasonposter == 0:  Log.Warn("Update() - TVDB - No poster, check logs in ../../Plug-in Support/Data/com.plexapp.agents.hama/DataItems/TVDB posters missing.htm to update Metadata Source")
    ...
    if not movie and (len(media.seasons)>2 or max(map(int, media.seasons.keys()))>1 or metadata_id_source_core == "tvdb"):
      ...
      if tvdbGenre:
        ...
        Log.Info("Update() - TVDB - tvdbGenre: '%s'" % str(tvdbgenre))
      list_eps = []
          ...
          if ep in tvdb_table:
            ...
            if 'FirstAired'  in tvdb_table[ep] and tvdb_table [ep] ['FirstAired']:
              ...
              if match:
                ...
                except ValueError, e: Log.Error("update - TVDB parseAirDate - Date out of range: " + str(e))
      ...
      ### AniDB Serie XML ##################################################################################################################################
      Log.Info("Update() - AniDB mode - AniDB Serie XML: " + ANIDB_HTTP_API_URL + metadata_id_number + ", " + "AniDB/"+metadata_id_number+".xml" )
      ...
      except Exception as e:  Log.Error("Update() - AniDB Serie XML: Exception raised, probably no return in xmlElementFromFile, Exception: '%s'" % e)
      if not anime:
        try:
        ...
        except Exception as e:  Log.Error("Update() - AniDB Title: Exception raised, Exception: '%s'" % e)
        else:  # title, orig = title.encode("utf-8").replace("`", "'"), orig.encode("utf-8").replace("`", "'")
          if title == str(metadata.title):  Log.Info("Update() - AniDB title: '%s', original title: '%s', metadata.title '%s'*" % (title, orig, metadata.title))
          elif title != "": #If title different but not empty [Failsafe]
            Log.Info("Update() - AniDB title: '%s', original title: '%s', metadata.title '%s'" % (title, orig, metadata.title))
        ...
        ### AniDB Start Date ###
        if getElementText(anime, 'startdate') == "":                                  Log.Info("Update() - AniDB Start Date: None")
        elif metadata.originally_available_at == getElementText(anime, 'startdate'):  Log.Info("Update() - AniDB Start Date: '%s'*" % str(metadata.originally_available_at))
        else:
          ...
          Log.Info("Update() - AniDB Start Date: '%s'" % str(metadata.originally_available_at))
        
        ### AniDB Ratings ###
        misc = getElementText(anime, 'ratings/permanent')
        if misc=="":                                         Log.Info("update() - AniDB Ratings:    'None'")   
        elif '.' in misc and float(misc) == metadata.rating: Log.Info("update() - AniDB Ratings:    '%s'*" % misc)
        else:                                                Log.Info("update() - AniDB Ratings:    '%s'"  % misc);  metadata.rating = float( misc )
        ...
        if sorted(metadata.genres)==sorted(genres): Log.Info(log_string+str(sortedGenres)+"*") 
          ...
          Log.Info("Update() - genres: " + str(sortedGenres) + " " + str(genres))
          metadata.genres.clear()
        ...
        ### AniDB Posters ###
        Log.Info("Update() - AniDB Poster, url: '%s'" % (ANIDB_PIC_BASE_URL + getElementText(anime, 'picture')))
        ...
        if not movie: ### TV Serie specific #################################################################################################################
          ...
          for episode in anime.xpath('episodes/episode'):   ### Episode Specific ###########################################################################################
            ...
            if epNumType=="3":
              ...
              Log.Info("Update() - AniDB specials title - Season: '%s', epNum.text: '%s', epNumVal: '%s', ep_title: '%s'" % (season, epNum.text, epNumVal, ep_title) )
            ...
            if episodeObj.title == ep_title:  Log.Info("Update() - AniDB episode title: '%s'*" % ep_title) 
            else:                             Log.Info("Update() - AniDB episode title: '%s'"  % ep_title); episodeObj.title = ep_title
            ...
            if airdate:
              ...
              if match:
                ...
                except ValueError, e:  Log.Error("Update() - AniDB parseAirDate - Date out of range: " + str(e))
            if originally_available_at == episodeObj.originally_available_at: Log.Info("Update() - AniDB AirDate '%s'*" % airdate)
            else:                                                             Log.Info("Update() - AniDB AirDate '%s'"  % airdate);  episodeObj.originally_available_at = originally_available_at
            ...
            if getElementText(episode, 'length'):
              ...
              if episodeObj.duration == duration:  Log.Info("Update() - AniDB duration: '%d'*" % duration)
              else:                                Log.Info("Update() - AniDB duration: '%d'"  % duration);  episodeObj.duration = duration;               
          ...
          Log.Info("Update() - DURATION: %s, numEpisodes: %s" %(str(totalDuration), str(numEpisodes)) )
  ...
  def anidbTvdbMapping(self, metadata, anidb_id, error_log):
    ...
    for anime in AniDB_TVDB_mapping_tree.iter('anime') if AniDB_TVDB_mapping_tree else []:
      ...
      if anidbid == anidb_id: #manage all formats latter
        ...
        Log.Info("anidbTvdbMapping() - anidb: '%s', tvbdid: '%s', tmdbid: '%s', imbdid: '%s', studio: '%s', defaulttvdbseason: '%s', name: '%s'" % (anidbid, tvdbid, tmdbid, imdbid, mapping_studio, defaulttvdbseason, name) )
        anidbid_table = []
  ...
  def anidbCollectionMapping(self, metadata, anime, anidbid_table=[]):
    ...
    for element in AniDB_collection_tree.iter("anime") if AniDB_collection_tree else []:
      if element.get('anidbid') in related_anime_list + anidbid_table + [metadata_id_number] :
        ...
        Log.Info("anidbCollectionMapping() - anidbid '%s' is part of movie collection: %s', related_anime_list: '%s', " % (metadata_id_number, title, str(related_anime_list)))
        return
    Log.Info("anidbCollectionMapping() - anidbid is not part of any collection, related_anime_list: '%s'" % str(related_anime_list)) 
  ...
  def getImagesFromASS(self, metadata, media, tvdbid, movie, num=0):
    ...
    except Exception as e:  Log.Error("getImagesFromASS() - Issues in finding setup info as directories have most likely changed post scan into Plex, Exception: '%s'" % e)
    try:                    postersXml = XML.ElementFromURL( ASS_POSTERS_URL, cacheTime=CACHE_1HOUR * 24)
    except Exception as e:  Log.Error("getImagesFromASS() - Loading poster XML failed: '%s', Exception: '%s'"% (ASS_POSTERS_URL, e)); return
    else:                   Log.Info("getImagesFromASS() - Loaded poster XML: '%s'" % ASS_POSTERS_URL)
    entry = postersXml.xpath("/tvdb4entries/posters[@tvdbid='%s']" % tvdbid)
    if not entry: Log.Error("getImagesFromASS() - tvdbid '%s' is not found in xml file" % tvdbid); return
  ...
  def getImagesFromTVDB(self, metadata, media, tvdbid, movie, poster_id=1, force=False, defaulttvdbseason_offset="", num=0):
    ...
    except Exception as e:  Log.Error("getImagesFromTVDB() - Loading picture XML failed: '%s', Exception: '%s'" %(TVDB_BANNERS_URL % tvdbid, e)); return
    else:                   Log.Info("getImagesFromTVDB() - Loaded picture XML: '%s'" % (TVDB_BANNERS_URL % tvdbid))
  ...
  def  getImagesFromTMDB(self, metadata, id, num=90):
    ...
    if id.startswith("tt"):
      Log.Info("getImagesFromTMDB() - using IMDBID url: " + TMDB_SEARCH_URL_BY_IMDBID % id)
      ...
      Log.Info("getImagesFromTMDB() - using TMDBID  url: " + TMDB_IMAGES_URL % id)
  ...
  def getImagesFromOMDB(self, metadata, imdbid, num=99):
    Log.Info("getImagesFromOMDB() - imdbid: '%s', url: '%s', filename: '%s'" % (imdbid, OMDB_HTTP_API_URL + imdbid, "OMDB/%s.jpg" % imdbid))
    try:                   OMDB = self.get_json(OMDB_HTTP_API_URL + imdbid, cache_time=CACHE_1WEEK * 56)
    except Exception as e: Log.Error("getImagesFromOMDB() - Exception - imdbid: '%s', url: '%s', filename: '%s', Exception: '%s'" % (imdbid, OMDB_HTTP_API_URL + imdbid, "OMDB/%s.jpg" % imdbid, e))
    else:
      ...
      else:                                                                      Log.Info("getImagesFromOMDB() - No poster to download - " + OMDB_HTTP_API_URL + imdbid)
  ...
  def metadata_download (self, metatype, url, num=99, filename="", url_thumbnail=None):  #if url in metatype:#  Log.Debug("metadata_download - url: '%s', num: '%s', filename: '%s'*" % (url, str(num), filename)) # Log.Debug(str(metatype))   #  return
    if url not in metatype:
      ...
      if filename and Data.Exists(filename):  ### if stored locally load it
        ...
        except Exception as e:  Log.Warn("metadata_download() - media_download - could not load file '%s' present in cache, Exception: '%s'" % (filename, e))
      if file == None: ### if not loaded locally download it
        ...
        except Exception as e:  Log.Error("metadata_download() - metadata_download - error downloading, Exception: '%s'" % e); return
        else:  ### if downloaded, try saving in cache but folders need to exist
          if filename and not filename.endswith("/"):
            try:                    Data.Save(filename, file)
            except Exception as e:  Log.Error("metadata_download() - metadata_download - could not write filename '%s' in Plugin Data Folder, Exception: '%s'" % (filename, e)); return
      if file:
        ...
        except Exception as e:  Log.Error("metadata_download() - metadata_download - issue adding picture to plex - url downloaded: '%s', filename: '%s', Exception: '%s'" % (url_thumbnail if url_thumbnail else url, filename, e)) #metatype.validate_keys( url_thumbnail if url_thumbnail else url ) # remove many posters, to avoid
        else:                   Log.Info("metadata_download() - url: '%s', num: '%d', filename: '%s'" % (url, num, filename))
    else:  Log.Info("metadata_download() - url: '%s', num: '%d', filename: '%s'*" % (url, num, filename))
  ...
  def get_json(self, url, cache_time=CACHE_1MONTH):
    ...
    except Exception as e:  Log.Error("get_json() - Error fetching JSON url: '%s', Exception: '%s'" % (url, e))
  ...
  def xmlElementFromFile (self, url, filename="", delay=True, cache=None):
    Log.Info("xmlElementFromFile() - url: '%s', filename: '%s'" % (url, filename))
    ...
    except Exception as e:  result = None; Log.Warn("xmlElementFromFile() - XML issue loading url: '%s', Exception: '%s'" % (url, e))                                                           # issue loading, but not AniDB banned as it returns "<error>Banned</error>"
    
    if result and len(result)>1024 and filename:  # if loaded OK save else load from last saved file
      try:                    Data.Save(filename, result)
      except Exception as e:  Log.Warn("xmlElementFromFile() - url: '%s', filename: '%s' saving failed, probably missing folder, Exception: '%s'" % (url, filename, e))
    elif filename and Data.Exists(filename):  # Loading locally if backup exists
      Log.Info("xmlElementFromFile() - Loading locally since banned or empty file (result page <1024 bytes)")
      try:                    result = Data.Load(filename)
      except Exception as e:  Log.Error("xmlElementFromFile() - Loading locally failed but data present - url: '%s', filename: '%s', Exception: '%s'" % (url, filename, e)); return
    ...
    if url==ANIDB_TVDB_MAPPING and Data.Exists(ANIDB_TVDB_MAPPING_CUSTOM):  # Special case: if importing anidb tvdb mapping, load custom mapping entries first
      Log.Info("xmlElementFromFile() - Loading local custom mapping - url: '%s'" % ANIDB_TVDB_MAPPING_CUSTOM)
    ...
    if result:
      ...
      if str(element).startswith("<Element error at "):  Log.Error("xmlElementFromFile() - Not an XML file, AniDB banned possibly, result: '%s'" % result)
      else:                                              return element
