  ...
  if serie_json:
    ...
    for index in sorted_episodes_index_list:
      ...
      if not(season =='0' and episode in list_sp_eps) and \
        ...
        if int(time.strftime("%Y%m%d")) <= air_date+1:  pass # Log.Info("TheTVDB.GetMetadata() - Episode '{}' missing but not aired/missing '{}'".format(numbering, air_date))
        ...
        Log.Info('[ ] numbering: {:>7} s{:0>2}e{:0>3} air_date: {}'.format(numbering, season, episode, Dict(episode_json, 'FirstAired')))
        
      ### File present on disk
      else:
        #Log.Info('[?] episode_json: {}'.format(episode_json))
        Log.Info('[X] numbering: {:>7} s{:0>2}e{:0>3} abs_number: {}, title: {}'.format(numbering, season, episode, abs_number, Dict(episode_json, 'episodeName')))
