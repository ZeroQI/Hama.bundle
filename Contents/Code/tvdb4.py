...
def GetMetadata(media, movie, source, TVDBid, mappingList, num=0):
  ...
  else:             TVDB4_mapping = TVDB4_mapping or common.LoadFile(filename=os.path.basename(TVDB4_MAPPING_URL), url=TVDB4_MAPPING_URL, cache=CACHE_1DAY*6)  # AniDB title database loaded once every 2 weeks
  entry = ""
  ...
  else:         TVDB4_xml  = TVDB4_xml or common.LoadFile(filename=os.path.basename(TVDB4_POSTERS_URL), url=TVDB4_POSTERS_URL, cache=CACHE_1DAY*6)  # AniDB title database loaded once every 2 weeks
  if TVDB4_xml:
    seasonposternum = 0
